...
static const char* const kComponent[MHI_DISCOVERY_ROWS] = {
  ...
  "sensor", "button", "sensor", "binary_sensor", "binary_sensor"};
...
// external-Troom row. MhiDiscoveryRow is append-only: whoever appends a row decides
...
static_assert(MHI_DISCOVERY_TROOM_EXTERNAL + 1 == MHI_DISCOVERY_ROWS,
              "a row appended after MHI_DISCOVERY_TROOM_EXTERNAL: classify it in mhi_discovery_is_outdoor_row() first");
...
size_t mhi_discovery_build(MhiDiscoveryRow row, const MhiDiscoveryCtx* c, char* out, size_t out_len) {
  ...
  switch (row) {
    case MHI_DISCOVERY_CLIMATE: {
      ...
      // 10: heat accepts the night-setback range; the firmware refuses below
      // 18 in every other mode (fork #25 F3, mhi_setpoint.h).
      put(&o, FMT("\"min_temp\":10,\"max_temp\":30,\"temp_step\":0.5,"));
      break;
    }
    ...
    case MHI_DISCOVERY_RESTART:
      ...
      break;
    case MHI_DISCOVERY_RUN_TIME:
      // The AC's own counter of this indoor unit, in hours (100 h steps), under
      // the unit's own operating-data prefix (fork #27). Home Assistant turns
      // it into the filter-cleaning reminder.
      put(&o, FMT("\"stat_t\":\"~/%s%s\",\"dev_cla\":\"duration\",\"unit_of_meas\":\"h\",\"stat_cla\":\"total_increasing\","),
          c->unit_op_prefix, c->t_op_total_iu_run);
      break;
    case MHI_DISCOVERY_CLEANING:
      // Allergen Clear: the unit reads off while its fan runs for 1.5 h (fork #25).
      put(&o, FMT("\"stat_t\":\"~/%s\",\"pl_on\":\"%s\",\"pl_off\":\"%s\",\"dev_cla\":\"running\","),
          c->t_cleaning, c->cleaning_on, c->cleaning_off);
      break;
    case MHI_DISCOVERY_TROOM_EXTERNAL:
      // On while a fresh set/Troom value is the AC's room temperature, Off once
      // it fell back to its own sensor (fork #25 C3).
      put(&o, FMT("\"stat_t\":\"~/%s\",\"pl_on\":\"%s\",\"pl_off\":\"%s\",\"ic\":\"mdi:thermometer-check\","),
          c->t_troom_external, c->troom_external_on, c->troom_external_off);
      break;
