...
- a boot-time MISO fault leaves MISO an input instead of driving against whatever else drives the line; the unit stays on WiFi/MQTT/OTA and keeps reading status, the AC just gets no frames (error state after ~120 s), instead of v2.8's reboot loop
- bug sweep (#10): Wi-Fi scan gets a timeout instead of hanging forever; `WIFI_LOST`/`MQTT_LOST` counters actually count; `set/Tsetpoint` acks on `cmd_received`; `Troom` resends after an MQTT reconnect; `set/PassiveMode` rejects unknown payloads; `set/Mode` matches the published "off" text; DS18x20 re-applies after a Troom timeout and re-inits a replugged sensor in `Tds1820`-only builds
- new status topic `Action` (#16): what the AC is doing (`off`/`idle`/`cooling`/`heating`/`drying`/`fan`), decoded from DB13, in Home Assistant's `hvac_action` names
- MQTT reconnects paced to one per 5 s (#15), so a refusing broker or restart no longer resets Wi-Fi within milliseconds
- dependencies refreshed 2026-09-15: PubSubClient3 3.3.1, CI actions to current majors
- WiFi PHY mode fallback (#17): tries 802.11g after 5 min without a link (some routers refuse the default 802.11n, [#224](https://github.com/absalom-muc/MHI-AC-Ctrl/issues/224)), alternates back after another 5 min; new `WIFI_PHY` status topic
- periodic telemetry (#18): `RSSI`, new `Uptime` and `FreeHeap` published at MQTT connect and every `TELEMETRY_PERIOD` s; new `ResetReason` topic exposes silent watchdog reboots
- protocol discovery tooling (#4, batch A): `diag/frame` (changed status bytes), `diag/opdata` (undecoded operating data with value), `set/OpDataRequest` (ask for any code) — turns "what does this button do" into a five-minute test
- named vanes, Silent and HA discovery (#4, batch B): `Vanes` publishes named positions and accepts them on `set/Vanes`; new `Silent` topic/`set/Silent` (opdata `0xDD`); with `HA_DISCOVERY`, climate + vane select + Silent switch + problem/wiring sensors are published after every MQTT connect
- discovery entity IDs (#4): `HA_ENTITY_PREFIX` pins `default_entity_id` so IDs survive an area assignment or lost registry
- left/right louvers, 3D auto, outdoor device, counters (#20, #19, #21): `VanesLR` named positions; `HA_OUTDOOR_DEVICE` gives the shared outdoor unit its own HA device via `via_device`; new `FrameErrors`/`FrameTimeouts`; named `Fan` levels; HA sensors for `Errorcode`/`OpData/PROTECTION-NO`, tabulated in `SW-Configuration.md`
- outdoor election, crash-loop safe mode, ride-alongs (#22, #23, #24): indoor units sharing an outdoor unit elect one (via `<GROUP_ROOT>members/` records) to publish the outdoor values and its HA configs; `Group` topic + sensor show each unit's role; `HA_OUTDOOR_DEVICE` removed, `KWH` stays per unit, `TELEMETRY_PERIOD` must be 1..86400. Three crashes within 120 s of boot trigger 10-minute safe mode (Wi-Fi/OTA only); `SafeMode` topic counts entries; `set/reset crash` tests it; new HA Restart button; `Discovery` reports `skipped` for configs that didn't fit
- outdoor device stays available while any group unit is connected (#29): its configs list every unit's `connected`, lowest hostname as `via_device`; `Group` reads `0` as soon as a unit connects
- vanes after the IR remote, Remote flag (#38, #39): `Vanes` shows the setting after a remote press too (no more `?`); new `Remote` topic is `On` when the remote made the last change; discovery adds `OpData/IU-FANSPEED`/`OpData/Tsetpoint` sensors (HA derives HI POWER/ECO from them)
- heating below 18 °C (#30): a heat target below 18 keeps the AC's setpoint at 18 and reaches the target via a shifted `set/Troom` (offset `HEAT_SHIFT_OFFSET`); `Tsetpoint` shows the target, `Troom` the room
- sweep (#33): `set/Troom`/`set/Tsetpoint` refuse non-numeric input, `set/Tsetpoint` refuses off-0.5-step values; `TOTAL-IU-RUN` publishes 0 too; `Tsetpoint` no longer double-publishes after a heat-shift ends; DS18x20-as-Troom publishes every step; debug texts moved to flash (~0.6 KB RAM)
