# Batch C: left/right louvers and 3D auto in Home Assistant, the outdoor device, frame counters, fan names, error codes
...
- Option `HA_OUTDOOR_DEVICE` (`#ifdef` switch, off by default; on for Slaapkamer in the toolkit), with `HA_OUTDOOR_ID` (default `"<HA_ID_PREFIX>_outdoor"`; Lucas: `ac_outdoor`, not tied to a room, so the publisher can move to the other unit without a new device), `HA_OUTDOOR_NAME` (default `"AC outdoor unit"`) and `HA_OUTDOOR_ENTITY_PREFIX` (Lucas: `ac_outdoor`, so the entities read `sensor.ac_outdoor_temperature`). Requires `HA_DISCOVERY`. Everything is English again as of 18 Sep (see §6): the units publish the repository's default entity names, so the outdoor rows are named "Temperature", "Current", "Energy", "Compressor frequency", "Defrost", "Compressor run time" and "Protection state" -- without "outdoor" in them, because the device already carries it.
...
| `MHI_DISCOVERY_OU_PROTECTION` | `protection` | sensor | `OpData/PROTECTION-NO` | diagnostic, the number as it is: no unit, no device class, no state class (§4.3) |
...
### 4.3 The error code and the protection number in Home Assistant (F2)
**Design change of 18 Sep (Lucas), replacing the "error text" of the first draft:** the firmware carries no error or protection texts at all. It publishes the numbers it already has -- `Errorcode` and `OpData/PROTECTION-NO` -- and Home Assistant does the wording. There is no `mhi_error_text` module, no `ErrorText` topic and no `OpData/PROTECTION-TEXT`.

Why: Home Assistant is the only consumer of these two topics, and a template sensor translates a number as well as C++ can; the "byte n means E n" mapping is MHI's numbering taken at face value and has never been verified against a unit's display, so a correction must not need a flash; and a table of texts is about 1.6 KB of DRAM on a device whose free heap is the thing this batch is measuring. The error path cannot be exercised on these units in the first place -- they have never reported a fault -- so untested text formatting would ship untried.

What the firmware gains is therefore only discovery: `MHI_DISCOVERY_ERROR_CODE` (`<id_prefix>_error_code`, sensor, diagnostic, no unit, no device class, no state class -- it is a code, not a measurement) on the existing `Errorcode` topic, next to the existing `Problem` binary sensor that templates the same topic; and the outdoor device's `MHI_DISCOVERY_OU_PROTECTION` on the existing `OpData/PROTECTION-NO`, likewise a plain diagnostic number. Entity names: `HA_NAME_ERROR_CODE` (default "Error code"), `HA_NAME_OU_PROTECTION` (default "Protection state").

The meanings live in the documentation and in hass-config. `SW-Configuration.md` gets the researched RAC table (23 codes) next to the compressor-protection table it already has, with its two sources (MHI / Beijer Ref *Service Support Handbook 2021.11*, RAC fault codes and the RAC multisplit table, cross-checked against the *SRK-ZSP-S Service Support Handbook*), the note that the PAC and KX tables of the same handbook use some of these numbers differently, the "byte = E number" caveat with the E1 observation from passive mode, and the statement that the fork publishes numbers only. The working table is kept in `.superpowers/sdd/2026-09-18-batch-c-louvers-outdoor-counters/error-table.md`. The dead `hrponline.co.uk` handbook links in `SW-Configuration.md` are replaced by the live `mhi-hvac.co.uk` one in the same step.
...
**Back to English (Lucas, 18 Sep):** the toolkit stops injecting Dutch entity names and the Dutch reset-reason template, so both units publish the repository's default English names and `default_entity_id` becomes `<domain>.<entity_prefix>_<slug of the default name>`. Home Assistant entity IDs therefore change on this flash, and hass-config is told which ones. `units.sh`: `FRAME=33` for both units. `airco-config.py`: drop the `--name-*` and `--reset-reason-tpl` overrides, `TELEMETRY_PERIOD 60`, the outdoor block for Slaapkamer only (`HA_OUTDOOR_ENTITY_PREFIX ac_outdoor`), the fan names once hass-config has chosen them (until then none, i.e. numeric), all covered by its tests; `--discovery-args` grows to match. `post-flash-check.sh`: the new retained topics (`FrameErrors`, `FrameTimeouts`, `VanesLR` a name), the new entities, the outdoor device on Slaapkamer, gated on the batch's first commit. `health-check.sh`: the two counters in the status line.
