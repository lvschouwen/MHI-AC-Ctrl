...
## 13. Amendment: the outdoor configs stop depending on the publisher (fork #29, 25 Sep 2026)

Agreed by Lucas, Codex and hass-config-09 (hass-config sandbox-tested the payload on HA 2026.9.3). It replaces the parts of §6.2 rule 3, §6.3, §8.2 and §8.4 named below. Everything else stands.

- **Why.** A publisher that reboots (flash, `set/reset`, Restart) is back in ~20 s, under the 30 s gone threshold, so no takeover happens and the six outdoor entities read unavailable for those 20 s: their availability was the publisher's `connected`. A planned hand-over was rejected: `lowest_candidate()` stops the peer claiming while a lower hostname is alive, the leaving unit would re-claim after settle, and the 30 s config delay leaves the gap anyway.
- **§8.2, availability.** The outdoor rows carry `"avty":[{"t":"<prefix>connected","pl_avail":"1","pl_not_avail":"0"},…],"avty_mode":"any"` instead of `avty_t` and the top-level `pl_avail`/`pl_not_avail`. HA reads the payloads per list entry and defaults to `online`/`offline`; the top-level ones apply only to `avty_t`. The list is `mhi_group_availability()`: this unit and every peer with a protocol-1 record and this unit's outdoor ID, alive or not; only an empty record removes one. It holds at most `MHI_GROUP_AVTY_MAX` (3) entries, this unit and the lowest other hostnames, sorted by hostname. The worst row, with three 64-character prefixes, measures 941 of 1024 B (host test). The outdoor device stays available while any unit is connected. Its values are retained and freeze while the publisher is away, and a real takeover still follows §6.2.
- **§8.2, `via_device`.** It is the first (lowest) hostname of the list, not the publisher's. With the same list, both units send byte-identical payloads, so a takeover changes nothing in HA. The key cannot be dropped: HA keeps the old `via_device_id` when a payload lacks it.
- **§6.3 and §6.2 rule 3.** The 30 s delay before the configs and rule 3's 35 s re-send are gone, and so is the state behind them. A claim and a publisher start ask for the configs in the same tick as `reset_system_values()`. On a first install the entities read unknown for up to one op-data cycle. A publisher asks again when its list has changed and then held still for `MHI_GROUP_AVTY_SETTLE_MS` (10 s). A record that changes only role, term or uptime leaves the list as it is. The 10 s also covers what rule 3 covered: after simultaneous claims, the loser's configs list only itself and go out at its claim; the winner's go out 10 s after the loser's record arrived.
- **§6.1, Group at connect.** `Group 0` goes out at the first tick after every connect, not at the end of the grace period, so a retained `Group 1` from before the connect never stands next to the real publisher's for 5 s. The system-value gate (§6.4) still waits for the grace period.
- **§11, flash order for this change.** Flash the member first, then the publisher. A member never writes outdoor configs. A publisher on an older build writes its old configs only at a claim or a publisher start, and the second flash replaces them. `post-flash-check.sh` compares the outdoor configs only when the publisher runs a build with the list.
